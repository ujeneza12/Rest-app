...
export default function Login() {
  ...
  const handleSubmit = (event) => {
    ...
    if (errors.email === "" && errors.password === "") {
      axios
        .post("http://localhost:8081/users/login", values)
        .then((res) => {
          if (res.status === 200) {
            navigate('/dashboard');
          } else {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="bg-white w-full max-w-md p-8 shadow-lg rounded-lg">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-[#0469a3c2]">Login</h1>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="text-right">
            <Link to="/forgot-password" className="text-[#0469a3c2]">Forgot password?</Link>
          </div>
          <button
            type="submit"
        ...
        <div className="text-center mt-6">
          <p>
            ...
            <Link to="/signup" className="text-[#0469a3c2]">Sign up</Link>
          </p>
        </div>
      </div>
    </div>
